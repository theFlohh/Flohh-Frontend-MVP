...
const MyTeam = () => {
  ...
  const totalUserPoints = team?.totalPoints;
  const weeklyPoints = team?.weeklyPoints;
...
const teamAvatar = team?.userProfileImage?.startsWith("http")
  ...
  : null;
  console.log("Team Avatar URL:", teamAvatar);
  // Just for debug
  useEffect(() => {
  ...
  const handleTeamNameSave = async () => {
    ...
    try {
      await updateDraft({ teamName: newTeamName.trim() }); // ✅ ab sahi jayega
      ...
      if (e.response?.status === 403) {
        setPopupMessage(e.response.data?.message || "❌ Draft is locked.");
      } else {
...
await updateDraft({ avatar }); 
      await updateDraft(formData, true);
  ...
  const createCustomTeamCardWithImages = async () => {
    ...
    ctx.fillText(
      `${totalUserPoints.toLocaleString()} Points`,
      canvas.width / 2,
      140
    );
    ...
    ctx.fillText(
      `Rank #${rank} • Weekly: ${weeklyPoints.toLocaleString()} pts`,
      canvas.width / 2,
      180
    );
    ...
    const imagePromises = members.slice(0, maxMembers).map((member) => {
      ...
      const imageUrl =
        artist.image || artist.avatar || artist.profileImage || null;
    ...
    for (let i = 0; i < maxMembers; i++) {
      ...
      ctx.textAlign = "right";
      ctx.fillText(
        `${(artist.totalScore || 0).toLocaleString()} pts`,
        canvas.width - 80,
        y - 5
      );
    ...
    if (members.length > 5) {
      ...
      ctx.textAlign = "center";
      ctx.fillText(
        `... and ${members.length - 5} more team members`,
        canvas.width / 2,
        remainingY
      );
    }
      ...
      if (imageData) {
        ...
        if (navigator.share && "canShare" in navigator) {
          try {
            ...
            console.log(
              "Native share failed, falling back to download + Twitter"
            );
          }
        }
        ...
        setPopupMessage(
          "🐦 Team image downloaded! Twitter opened - attach the downloaded image to your tweet."
        );
      } else {
  ...
  const shareOnInstagram = async () => {
    ...
    try {
      ...
      if (imageData) {
        ...
        document.body.removeChild(link);
        setPopupMessage(
          "📸 Team image downloaded! Open Instagram and attach the image to share."
        );
      } else {
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
                  ...
                  <div>
                    ...
                    <div className="text-sm sm:text-base font-semibold flex items-center gap-1 text-yellow-400">
                      ...
                      />
                      {weeklyPoints ? weeklyPoints.toLocaleString() : 0}
                    </div>
                    {userLeagueUrl ? (
                      <button
              ...
              <div className="border border-[#353751] bg-[#2a2d48] rounded-xl p-4 shadow-lg space-y-3 text-sm text-white">
                ...
                <div className="flex gap-2 mt-4 flex-wrap justify-center sm:justify-start">
                  ...
                  <div className="relative">
                    ...
                    <div
                      id="shareDropdown"
                      className="share-dropdown hidden absolute top-full mt-2 right-0 bg-[#2a2d48] border border-[#353751] rounded-lg shadow-lg z-10 min-w-40"
                    >
                      <button
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-row gap-2">
                          ...
                          <span className="bg-[#2a2d48] rounded-full px-2 py-1 text-xs mb-3 flex items-center gap-1">
                            {artist.currentRank !== undefined &&
                            artist.previousRank !== undefined ? (
                              artist.currentRank < artist.previousRank ? (
                                <img
