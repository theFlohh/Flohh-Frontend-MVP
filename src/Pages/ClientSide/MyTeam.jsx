    ...
    <>
      ...
      <div className="min-h-screen text-white p-4 font-sans relative">
        <div className="max-w-6xl mx-auto relative">
          <div
            className="relative w-full h-32 md:h-40 rounded-2xl mb-25 overflow-hidden bg-cover bg-center"
            style={{ backgroundImage: `url('/img/banner-rect1.png')` }}
          ></div>
          <div className="absolute top-20 left-7 flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="relative w-40 h-40">
              <img
                src={avatarPreview || teamAvatar || "/img/default-avatar.png"}
                alt={teamName || "Team Avatar"}
                className="w-40 h-40 rounded-full border-4 border-purple-400 object-cover shadow-lg"
              />

              {/* Upload / Save Button - Right side of image */}
              <div className="absolute top-3/4 -translate-y-1/2 flex flex-col gap-2">
                {!avatarFile ? (
                  <label
                ...
                {editingTeamName || !teamName ? (
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      className="h-9 px-3 rounded-md bg-[#2a2d48] border border-purple-500 focus:border-purple-400 text-white placeholder-gray-400 outline-none"
                    />
                    <button
                      onClick={handleTeamNameSave}
                      disabled={teamNameLoading}
                      className="h-9 px-4 bg-purple-500 hover:bg-purple-600 text-white rounded-md text-sm flex items-center justify-center"
